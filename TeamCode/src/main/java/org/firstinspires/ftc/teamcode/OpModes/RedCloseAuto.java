...
public class RedCloseAuto extends LinearOpMode {
    ...
    private final Pose pickup1Pose = new Pose(120,83, heading);
    private final Pose pickup2Pose = new Pose(125,60, heading);
    private final Pose pickup3Pose = new Pose(120,33, heading);
    ...
    public void runOpMode() throws InterruptedException {
        ...
        while (!isStopRequested()){
            if (stage == 0){
                ...
                elapsedTime.reset();
                stage++;
            } else if (stage == 1 && robot.follower.atParametricEnd()) {
                processor.override(new Shoot());
                elapsedTime.reset();
                stage++;

            } else if (stage == 2 && elapsedTime.milliseconds() > 2000) {
                processor.override(new IntakeBall(false));
                robot.follower.followPath(grabPickup1);
                elapsedTime.reset();
                stage++;

            } else if (stage == 3 && robot.follower.atParametricEnd()) {
            ...
            } else if (stage == 5 && elapsedTime.milliseconds() > 2000) {
                processor.override(new IntakeBall(false));
                robot.follower.followPath(grabPickup2);
                elapsedTime.reset();
                stage++;

            } else if (stage == 6 && robot.follower.atParametricEnd()) {
            ...
            } else if (stage == 8 && elapsedTime.milliseconds() > 2000) {
                processor.override(new IntakeBall(false));
            ...
            } else if (stage == 9 && robot.follower.atParametricEnd()) {
                ...
                elapsedTime.reset();
                stage++;
            } else if (stage == 11 && elapsedTime.milliseconds() > 1500) {
